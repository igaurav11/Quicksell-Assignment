...
export const loadGridData = (tickets, userData, grouping, ordering) => {
    let newTickets;
    ...
    switch (grouping) {
        ...
        case('users'): return groupByUsers(newTickets, userData);
        case('priority'): return groupByPriority(newTickets);
        default: return groupByUsers(newTickets, userData);
    }
}
...
const groupByUsers = (tickets, userData) => {
    const groups = tickets.reduce((res, ticket) => {
    ...
    // Sort the object keys by userId
    const sortedKeys = Object.keys(groups).sort();

    // Reconstruct the sorted object
    return sortedKeys.reduce((sortedRes, key) => {
        sortedRes[key] = groups[key];
        return sortedRes;
    }, {});
};
